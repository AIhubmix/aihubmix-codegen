...
export function realtimeFallback(lang: keyof typeof LANG_NAME): (ctx: RealtimeCtx) => string {
  ...
  return (ctx) => {
    const commitNote = sessionUsesVad(ctx)
      ? 'send {"type":"input_audio_buffer.commit"} to flush trailing audio (server VAD segments on pauses)'
      : 'send {"type":"input_audio_buffer.commit"} to finalize a segment';
    return `${prefix} Realtime transcription runs over a WebSocket session (not HTTP request/response).
